...
while 1:
    ...
    y_pred = fit_model(MLPClassifier(activation='relu', solver='adam', hidden_layer_sizes=(50, 50, 50), max_iter=100000, random_state=42))# 8_1_mlp_1


    #y_pred = [sum(i) for i in zip(*y_preds)]
    d_pred = dict(collections.Counter(y_pred))
    min_sum = 0#min(list(d_pred))

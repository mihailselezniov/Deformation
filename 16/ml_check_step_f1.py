...
source_f = 'ml_threads/11_1_mlp_2.txt'
with open(source_f, 'r') as f:
    threads = f.readlines()

print(len(threads))
tr = [list(map(int, t.replace('\n', '').split(','))) for t in threads]
n = 60
step = 5
#print(tr[:n])
#print(tr[n:n+step])

def make_all_blocks(n):
    blocks = []
    for i in range(int(n/step)):
        since = i * step
        blocks.append(tr[since : since+step])
    print(len(blocks))
    return blocks

def make_train_test(pop_index):
    train_test = []
    new_blocks = deepcopy(blocks)
    new_blocks.pop(pop_index)
    for i in range(len(new_blocks)-1):
        train_blocks = deepcopy(new_blocks)
        test_block = train_blocks.pop(i)
        join_train_blocks = []
        for b in train_blocks:
            join_train_blocks.extend(b)
        train_test.append([join_train_blocks, test_block])
    return train_test

def make_xy(block):
    np_b = np.array(block)
    return np_b[:, :-1], np_b[:, -1]

def get_sum_f1_metric():
    f1_metric = []
    for i in train_test:
        train_block, test_block = i[0], i[1]
        x_train, y_train = make_xy(train_block)
        x_test, y_test = make_xy(train_block)
    ...
    return sum(f1_metric)

for n in range(20, 500, step*2):
    blocks = make_all_blocks(n)
    train_test = make_train_test(pop_index=-1)
    f1_1 = get_sum_f1_metric()

    train_test = make_train_test(pop_index=-2)
    f1_2 = get_sum_f1_metric()

    print(1 if f1_2 > f1_1 else 0, f1_1, f1_2)

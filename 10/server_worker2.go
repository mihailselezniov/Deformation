...
func init_fiber(length, diameter, young, density, pressure_time, pressure_radius, pressure_amplitude, strength float64) *Fiber {
	...
	f.POINTS_PER_FIBER, f.NUMBER_OF_FRAMES = 200, 45000
...
func start() {
	...
	req.res = strings.Trim(strings.Join(strings.Fields(fmt.Sprint(count_state_arr)), ","), "[]")
}

func calc_fib(w http.ResponseWriter, r *http.Request) {
	task := r.URL.Path
	...
	w.Write([]byte(req.res))
	return
}

func main() {
	http.HandleFunc("/", calc_fib)

...
func (s *State) dis_to_zero(point []float64) float64 {
    _dis := 99999.0
    for i := 0; i < s.len_zero; i++ {
        _dis = math.Min(s.hypot(s.zero[i], point), _dis)
    }
    return _dis
}

func (s *State) sort_one() {
    sort.SliceStable(s.one, func(i, j int) bool {
        return s.dis_to_zero(s.one[i]) < s.dis_to_zero(s.one[j])
    })
}
...
func main() {
    ...
    for i := 0; i < s.len_one; i++ {
        fmt.Println(s.dis_to_zero(s.one[i]), s.one[i])
    }

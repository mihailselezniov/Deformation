...
for cut in [100, 200, 300, 400, 500]:
    ...
    for t in threads[:cut]:
        ...
        x_train.append(tr[:-2])
        y_train.append(tr[-2])
        y_train2.append(abs(tr[-1]-2999)/2999)
        #print(tr[-2], abs(tr[-1]-2999)/2999)
    x_train, y_train = np.array(x_train), np.array(y_train2)
    ...
    def fit_model(model):
        ...
        lim = 0.1
        print('lim:', lim)
        y_pred = [1 if value > lim else 0 for value in y_pred]
    ...
    #fit_model(MLPRegressor(activation='relu', solver='adam', hidden_layer_sizes=(50, 50, 50), max_iter=100000, random_state=42))
    #fit_model(MLPRegressor(activation='relu', solver='adam', hidden_layer_sizes=(70, 70, 70), max_iter=100000, random_state=42))
    #fit_model(MLPRegressor(activation='tanh', solver='adam', hidden_layer_sizes=(90, 90, 90), max_iter=100000, random_state=42))
    fit_model(MLPRegressor(activation='tanh', solver='adam', hidden_layer_sizes=(70, 70, 70), max_iter=100000, random_state=42))

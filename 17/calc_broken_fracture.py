...
to_name = '13_1_mlp_4_fr_1.txt'
folder_name = 'ml_threads/'
f_name = folder_name + ex_name
f_name2 = folder_name + to_name
...
for t in threads:
    ...
    stream = os.popen('go run solve_fiber_fracture.go {}'.format(str_point))
    output = stream.read()
    #print(output, output.split()[-2:])
    row_data = list(point) + list(map(int, output.split()[-2:]))
    row_thread = ','.join(list(map(lambda x: str(int(x)), row_data)))
    print(row_thread)
    with open(f_name2, 'a') as f:
        f.write(row_thread + '\n')
